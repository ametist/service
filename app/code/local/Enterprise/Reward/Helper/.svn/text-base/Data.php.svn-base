...
class Enterprise_Reward_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function isCusotmerGotBonus($customerId, $comment)
    {
        #$comment = mysql_real_escape_string($comment);
        $customerId = (int)$customerId;
        $query =  "SELECT `history`.`comment` FROM `enterprise_reward` AS `main_table` 
        RIGHT JOIN `enterprise_reward_history` AS `history` ON main_table.reward_id = history.reward_id 
        WHERE (main_table.customer_id = '{$customerId}') AND (history.comment = '{$comment}')";
        
        $history = Mage::getSingleton('core/resource')->getConnection('core_read')->fetchAll($query); 
        
        return (count($history) >= 1) ? true : false; 
    }
}
?>
