...
class Enterprise_Reward_Model_Observer
{
    ...
    public function processPointForOrders(Varien_Event_Observer $observer)
    {
        $methods = array('rewardpay', 'sdfree');
        
        if (in_array($observer->getEvent()->getInvoice()->getOrder()->getPayment()->getMethodInstance()->getCode(), $methods)) {
            return;
        }
        
        $order = $observer->getEvent()->getInvoice()->getOrder();
        foreach($order->getAllItems() as $item){
            if($item->getProductId() == 362 || $item->getProductId() == 5323){
                return $this;
            }
        }
        $reward = Mage::getModel('enterprise_reward/reward')
        ...
        $oldQty = $reward->getCouponQty() ? $reward->getCouponQty() : 0;
        $rate = (int)(($qty + $oldQty) / 5);
        if($rate > 0){
            $diff = ($qty + $oldQty) % 5;
            $reward->setCouponQtyToUpdate($diff);
        }else{
            $reward->setCouponQtyToUpdate($qty + $oldQty);
        }        
        $amount = Mage::helper('enterprise_reward')->getPointsConfig('five_orders', Mage::app()->getWebsite()->getId()) * $rate;
        if($amount > 0 ){
            $reward->setPointsDelta($amount)
                ->setAction(Enterprise_Reward_Model_Reward::REWARD_ACTION_ADMIN)
                ->setComment('five_paid_coupons_reward/added for 5 paid coupons')
                ->updateRewardPoints();
        }else{
            $reward->save();
        }
        # $this->processPointForTellAFriend($observer, $reward);
        return $this;
    }
    
    /*public function processPointForTellAFriend($observer,$reward)
    {
        if($reward->getWasFirstOrder()) return;        
        $customer = Mage::getModel('customer/customer')->load($reward->getCustomerId());
        if(!($affiliateId = $customer->getAffiliateCodeId())) return;
        $affiliate = Mage::getModel('affiliate/affiliatecustomer')->loadByAffiliateId($affiliateId, 'affiliate_id');        
        if(!$affiliate->getId()) return;
        $rewardParent = Mage::getModel('enterprise_reward/reward');        
        $rewardParent->setCustomerId($affiliate->getCustomerId())
                ->setWebsiteId(Mage::app()->getWebsite()->getId())
                ->setPointsDelta(Mage::helper('enterprise_reward')->getPointsConfig('tell_a_friend', Mage::app()->getWebsite()->getId()))
                ->setAction(Enterprise_Reward_Model_Reward::REWARD_ACTION_ADMIN)
                ->setComment('tell_a_friend/added for first frient paid order')                
                ->updateRewardPoints();
        $reward->setWasFirstOrder(1)->save();
    }*/
}
